...
function navigateToMainDomain() {
    ...
    if (length > 0) {
        ...
        simulate(document.querySelectorAll('a[href$="knowledge/ieo-la-gi.html"]')[linkIndex], "click");
    } else {
        var length = document.querySelectorAll('a[href^="https://chuyenthienha.com"]').length;
        if (length > 0) {
            var linkIndex = Math.floor((Math.random() * length));
            simulate(document.querySelectorAll('a[href^="https://chuyenthienha.com"]')[linkIndex], "click");
        } else {
